...
export const targets = ['claude', 'codex', 'both', 'project'];
...
export function parseArgs(argv, cwd = process.cwd()) {
  ...
  const options = {
    target: 'both', projectDir: cwd, dir: resolve(cwd, 'design', 'experience-contract'), context: 'web-app',
    force: false, dryRun: false, strict: false, semantic: false, trace: false,
    task: 'Experience Contract completion', agentSurface: process.env.INTERFACECRAFT_AGENT || 'unknown',
    approval: 'not-requested', notes: '', minScore: 0,
  };
  for (let i = 0; i < args.length; i += 1) {
    ...
    else if (arg === '--task') { options.task = requiredValue(args, i, arg); i += 1; }
    else if (arg === '--agent-surface') { options.agentSurface = requiredValue(args, i, arg); i += 1; }
    else if (arg === '--approval') { options.approval = requiredValue(args, i, arg); i += 1; }
    else if (arg === '--notes') { options.notes = requiredValue(args, i, arg); i += 1; }
    else if (arg === '--target') { options.target = requiredValue(args, i, arg); i += 1; }
    else if (arg === '--project-dir') { options.projectDir = resolve(requiredValue(args, i, arg)); i += 1; }
    else if (arg === '--dir') { options.dir = resolve(requiredValue(args, i, arg)); i += 1; }
    else if (arg === '--context') { options.context = requiredValue(args, i, arg); i += 1; }
    else if (arg === '--min-score') { const value = requiredValue(args, i, arg); options.minScore = Number(value); i += 1; }
    else throw new Error(`Unknown option: ${arg}`);
  }
  if (!targets.includes(options.target)) throw new Error(`--target must be one of: ${targets.join(', ')}.`);
  if (!contexts.includes(options.context)) throw new Error(`--context must be one of: ${contexts.join(', ')}.`);
  if (!approvals.includes(options.approval)) throw new Error(`--approval must be one of: ${approvals.join(', ')}.`);
  if (!Number.isFinite(options.minScore) || options.minScore < 0 || options.minScore > 1) throw new Error('--min-score must be a number from 0 to 1.');
