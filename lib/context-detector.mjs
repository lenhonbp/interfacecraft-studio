...
export async function detectContext(projectDir, { minScore = 0 } = {}) {
  ...
  const meetsMinScore = winner.score >= minScore;
  return { projectDir, recommendedContext: meetsMinScore ? winner.context : null, bestCandidate: winner.context, confidence, margin, minScore, meetsMinScore, scores: ranking, reviewRequired: !meetsMinScore || confidence === 'low' || margin < 0.08, signals: names.slice(0, 80) };
}
