...
export async function verifyCompletion(projectDir, strict = false) {
  ...
  const valid = expectedHash === record.contentHash;
  const result = { path, valid, contentHash: record.contentHash, expectedHash, status: record.status, context: record.context, generatedBy: record.generatedBy };
  if (!valid && strict) throw new Error(`Completion record integrity check failed. Expected ${expectedHash}, found ${record.contentHash}.`);
